...
def download(filename):  # 所有分片均上传完后被调用
    ...
    if filename.endswith('xlsx'):
        res = make_response(send_from_directory(constants.DOWNLOAD_DIR, filename, as_attachment=True, conditional=True))
    elif filename.endswith('csv'):
        res = make_response(send_from_directory(constants.UPLOAD_DIR, filename, as_attachment=True, conditional=True))
    res.headers["Content-Disposition"] = 'attachment; filename={}"'.format(quote(filename))
    return res
...
def downloads(dir_path, filename):
    ...
    res = make_response(send_from_directory(dir_path, filename, as_attachment=True, conditional=True))
    res.headers["Content-Disposition"] = 'attachment; filename={}"'.format(quote(filename))
    return res
