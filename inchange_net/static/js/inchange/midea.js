...
function file_page() {
    // console.log('1');
    var partner = $('#ok span').html();
    // console.log(partner);
    $.ajax({
        url: "/api/v1_0/downpages",
        type: "post",
        dataType: "json",
...
window.onload = function () {
    ...
    // var ok = document.querySelector('#ok');
    // var run = document.querySelector('.run');
    // var partner_name = document.querySelector('.partner');
    // var after = document.querySelector('.run .after');

    $.ajax({
        url: '/api/v1_0/partners/midea',
        type: 'get',
        dataType: 'json',
        success: function (info) {
            if (info.status == 0) {
				// console.log(info.partner);
                $('.partner').html(info.partner);
                // console.log($('.partner').html());
                bars();
            } else {
                localtion.href = '/midea.html';
            }
        }
    });
